...
func NewPlaceOrder(c *common.Client, category, symbol, side, orderType string, qty float64) (*PlaceOrder, error) {
	order, err := model.NewOrder(category, symbol, side, orderType, qty)
	if err != nil {
		return nil, err
	}
